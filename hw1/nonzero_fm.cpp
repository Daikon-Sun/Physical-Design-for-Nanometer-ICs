...
inline bool is_good(int cid) {
  if( cell_list[cid].inA() ) return A-1>=(1-r)*tot_cell/2 && B+1<=(1+r)*tot_cell/2;
  else return B-1>=(1-r)*tot_cell/2 && A+1<=(1+r)*tot_cell/2;
  __builtin_unreachable();
}
inline void parse(const char* filename) {
  for(int i = 0; i<2; ++i) {
    int tot_net = 0, max_cellID = 0;
...
inline void random_split() {
  ...
  if(choice == 0) {
    for(int i = 0, cnt = 0; cnt<tot_cell/2; ++i) {
      ...
      ++cnt;
      const vector<int>& nets = cell_list[i].nets();
    ...
    for(int i = 0; i<(int)cell_list.size(); ++i) {
      if(cell_list[i].empty()) continue;
      cell_size.push_back(make_pair(get_total(i), i));
    }
...
inline void init() {
  for(size_t i = 0; i<cell_list.size(); ++i) if( !cell_list[i].empty() ) {
    ...
    auto it = gain_list[g].insert(gain_list[g].end(), i);
    cell.set_gain(g);
    cell.set_pos(it);
  }
}
...
inline void update(Net& net, bool to_from, bool inc, list<int>& changed_cell) {
  ...
  else if(net.get_cnt(to_from) == 1) change_only(net, to_from, !inc, changed_cell);
}
